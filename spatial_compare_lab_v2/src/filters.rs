...
impl FftConvolver {
    ...
    pub fn process(&mut self, block: &[f32]) -> Vec<f32> {
        ...
        let mut work: Vec<Complex<f32>> = self
            .input_buffer
            .iter()
...
mod tests {
    use super::*;
    #[test]
    fn overlap_save_matches_direct_convolution_across_blocks() {
        let h: Vec<f32> = (0..701).map(|i| (i as f32 * 0.13).sin() * 0.01).collect();
        let mut x: Vec<f32> = (0..513).map(|i| (i as f32 * 0.31).cos()).collect();
        x.resize(1536, 0.0);
        let mut conv = FftConvolver::new(&h, 256);
        let actual: Vec<f32> = x.chunks(256).flat_map(|b| conv.process(b)).collect();
        for n in 0..x.len() {
            let expected: f32 = (0..h.len().min(n + 1)).map(|k| h[k] * x[n - k]).sum();
            assert!((actual[n] - expected).abs() < 1e-5, "sample {n}");
        }
    }
}
