...
impl EnvironmentKind {
    ...
    pub fn slug(self) -> &'static str {
        match self {
            ...
            Self::Concertgebouw => "concertgebouw",
        }
    }
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            Self::Theater => "Theater",
            Self::Concertgebouw => "Concertgebouw-inspired Grand Hall",
        }
    }
}
...
impl RoomConfig {
    /// Override late-field controls for the web renderer; zero gain disables it.
    pub fn set_web_reverb(&mut self, rt60: f32, predelay: f32, gain: f32) {
        let mut late = self.late_params();
        late.enabled = gain > 0.0;
        late.rt60_s = rt60;
        late.rt60_high_s = rt60 * 0.75;
        late.predelay_ms = predelay;
        late.output_gain = gain;
        self.late_override = Some(late);
    }
    ...
    pub fn preset(kind: EnvironmentKind) -> Self {
        match kind {
            ...
            // Simplified shoebox inspired by the Grote Zaal, not a measured
            // reconstruction. Explicit decay avoids deriving hall acoustics
            // from the six coarse surface materials alone.
            EnvironmentKind::Concertgebouw => Self {
                environment: kind,
                width_m: 27.8,
                depth_m: 44.0,
                height_m: 17.5,
                ...
                max_er_ms: 160.0,
                air_hf_loss_db_per_m: 0.035,
                left: SurfaceSpec::on(PLASTER),
                right: SurfaceSpec::on(PLASTER),
                back: SurfaceSpec::on(PLASTER),
                front: SurfaceSpec::on(DIFFUSIVE_WOOD),
                floor: SurfaceSpec::on(WOOD_FLOOR),
                ceiling: SurfaceSpec::on(PLASTER),
    ...
    pub fn validate(&self) -> Result<(), Box<dyn std::error::Error>> {
        for az in [self.left_speaker_az_deg, self.right_speaker_az_deg] {
            let a = az.to_radians();
            let x = self.listener_x + self.speaker_distance_m * a.sin();
            let y = self.listener_y + self.speaker_distance_m * a.cos();
            if x <= 0.0 || x >= self.width_m || y <= 0.0 || y >= self.depth_m {
                return Err(format!(
                    "Speaker outside {} geometry at distance {} m and angle {} deg",
                    self.environment.label(),
                    self.speaker_distance_m,
                    az
                )
                .into());
            }
    ...
    // Direct pressure follows 1/r (1 m reference), including source height.
    // ER already includes 1/path in generate_events: do not attenuate it twice.
    // Hold diffuse excitation constant for fixed source power. late_mix remains
    // an artistic preset level, not a calibrated room-energy normalization.
    pub fn distance_cues(&self) -> DistanceCues {
        let d = self
            .speaker_distance_m
            .hypot(self.speaker_z - self.listener_z)
            .max(0.15);
        let direct_gain = (1.0 / d).clamp(0.08, 1.75);
        let early_gain = 1.0;
        let late_send = 1.0;
        DistanceCues {
            distance_m: d,
    ...
    pub fn late_params(&self) -> LateParams {
        ...
        let rt60_mid = if a_mid > 1e-3 {
            eyring_rt60(v, s_total, a_mid).clamp(0.20, 3.50)
        } else {
            3.50
        };

        let rt60_high = if a_high > 1e-3 {
            eyring_rt60(v, s_total, a_high).clamp(0.15, rt60_mid)
        } else {
            rt60_mid
        };
...
impl RoomBaker {
    ...
    pub fn bake_speaker(
        ...
        profile: &HrtfProfile,
        speaker_az_deg: f32,
        direct_hrtf_intensity: f32,
    ) -> BakedSpeaker {
        // Room renders pass 1.0 and therefore keep the direct HRTF exactly as
        // measured. The separate 02 comparison deliberately passes 0.90.
        let direct = profile.hrir_with_intensity(speaker_az_deg, direct_hrtf_intensity);
        let direct_l = direct.left;
        let direct_r = direct.right;
        ...
        for ev in &events {
            // Reflections use the original directional HRTF, followed by the
            // room material response. The HRTF profile itself is never blended.
            let h = profile.hrir(ev.azimuth_deg);
    ...
    fn generate_events(&self, speaker_az_deg: f32) -> Vec<ReflectionEvent> {
        ...
        for c in candidates {
            ...
            // Absolute path spreading. Previously direct_dist/path made ER grow
            // relative to an unattenuated direct signal in a non-physical way.
            let gain = (1.0 / path.max(0.15)) * specular;
            if gain < 0.015 {
                continue;
            }
...
// Eyring uses area-weighted energy absorption. It avoids Sabine's small-
// absorption approximation for treated rooms, but still assumes a diffuse field.
// Existing decay floors/ceilings remain artistic limits for this compact FDN.
fn eyring_rt60(volume: f32, surface_area: f32, absorption_area: f32) -> f32 {
    let mean_absorption = (absorption_area / surface_area).clamp(1e-6, 0.999999);
    0.161 * volume / (-surface_area * (-mean_absorption).ln_1p())
}
...
mod tests {
    ...
    #[test]
    fn distance_does_not_attenuate_early_paths_twice() {
        let near = RoomConfig::default().with_distance(1.0).distance_cues();
        let far = RoomConfig::default().with_distance(2.0).distance_cues();
        assert!((far.direct_gain / near.direct_gain - 0.5).abs() < 1e-6);
        assert_eq!(near.early_gain, 1.0);
        assert_eq!(far.early_gain, 1.0);
        assert_eq!(near.late_send, far.late_send);
    }

    #[test]
    fn direct_distance_includes_height() {
        let mut cfg = RoomConfig::default().with_distance(3.0);
        cfg.speaker_z = cfg.listener_z + 4.0;
        let cues = cfg.distance_cues();
        assert!((cues.distance_m - 5.0).abs() < 1e-6);
        assert!((cues.direct_gain - 0.2).abs() < 1e-6);
    }

    #[test]
    fn eyring_matches_uniform_room_and_low_absorption_limit() {
        // 5 m cube, surface 150 m2, uniform energy absorption 0.5.
        let expected = 0.161 * 125.0 / (150.0 * 2.0_f32.ln());
        assert!((eyring_rt60(125.0, 150.0, 75.0) - expected).abs() < 1e-6);
        let sabine = 0.161 * 125.0 / 1.5;
        assert!((eyring_rt60(125.0, 150.0, 1.5) / sabine - 1.0).abs() < 0.006);
    }

    #[test]
    fn presets_have_valid_geometry_and_bounded_reflections() {
        for kind in EnvironmentKind::all() {
            let cfg = RoomConfig::preset(kind);
            cfg.validate().unwrap();
            let late = cfg.late_params();
            assert!(late.rt60_s.is_finite() && late.rt60_high_s <= late.rt60_s);
            let baker = RoomBaker::new(48000, cfg);
            for az in [-30.0, 30.0] {
                let events = baker.generate_events(az);
                assert!(events.len() <= baker.cfg.max_events);
                for ev in events {
                    assert!(ev.gain.is_finite() && ev.gain > 0.0);
                    assert!(ev.delay_samples as f32 <= baker.cfg.max_er_ms * 48.0 + 0.5);
                }
            }
        }
    }
